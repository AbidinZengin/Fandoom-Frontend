#!/usr/bin/env node
// Pluribus'un Season 1 / bölüm verisini (görsel HARİÇ — sadece
...
// (create-pluribus.mjs önce çalıştırılmış olmalı).
//
// Kullanım (PowerShell):
//   $env:AUTH_TOKEN="..."; node scripts/seed-pluribus-seasons.mjs
...
function loadReport() {
  const path = resolve('scripts/output/pluribus-seed-report.json');
...
function saveReport(report) {
  ...
  writeFileSync(resolve('scripts/output/pluribus-seed-report.json'), JSON.stringify(report, null, 2));
}
...
async function main() {
  ...
  if (!seriesRes.ok) {
    throw new Error(`Pluribus series bulunamadı (slug: ${SERIES_SLUG}) — önce create-pluribus.mjs çalıştırılmalı.`);
  }
  const SERIES_ID = (await seriesRes.json()).id;

  const { seasons } = JSON.parse(readFileSync(resolve('scripts/data/pluribus-seasons.json'), 'utf8'));
  ...
  console.log('\nBitti. Rapor: scripts/output/pluribus-seed-report.json');
}
