#!/usr/bin/env node
// FROM'un sezon/bölüm verisini (görsel HARİÇ — sadece
// title/airDate/synopsisTr) POST /series/{id}/seasons ve
// POST /seasons/:id/episodes ile yazan tek seferlik seed script'i.
// seed-pluribus-seasons.mjs'in aynı deseni: idempotent, rapor
// dosyasından resume. SERIES_ID sabit değil — slug'dan otomatik çözülür
// (create-from.mjs önce çalıştırılmış olmalı).
//
// Kullanım (PowerShell):
//   $env:AUTH_TOKEN="..."; node scripts/seed-from-seasons.mjs
...
function loadReport() {
  const path = resolve('scripts/output/from-seed-report.json');
...
function saveReport(report) {
  ...
  writeFileSync(resolve('scripts/output/from-seed-report.json'), JSON.stringify(report, null, 2));
}
...
async function main() {
  ...
  if (!seriesRes.ok) {
    throw new Error(`FROM series bulunamadı (slug: ${SERIES_SLUG}) — önce create-from.mjs çalıştırılmalı.`);
  }
  const SERIES_ID = (await seriesRes.json()).id;

  const { seasons } = JSON.parse(readFileSync(resolve('scripts/data/from-seasons.json'), 'utf8'));
  ...
  console.log('\nBitti. Rapor: scripts/output/from-seed-report.json');
}
