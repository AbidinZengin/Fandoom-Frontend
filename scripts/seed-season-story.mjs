#!/usr/bin/env node
// SeasonStory.data.js'teki (frontend-only mock, sadece İngilizce) sezon
// "derin okuma" içeriğini backend'in yeni seasonBlocks[] + storyKicker/
// storyTitle/storyDek (+Tr ikizleri) alanlarına taşır. TR çevirisi
// scripts/data/breaking-bad-season-story-tr.json'dan gelir (editorial-voice:
// çeviri değil yeniden yazım). İngilizce içerik SeasonStory.data.js'ten
// canlı import edilir — tek kaynak, kopya veri yok.
//
// PUT bir SeasonRequest'tir ve seasonBlocks HARİÇ tüm alanlar skaler
// replace'tir (backend notu: "seasonBlocks null ise mevcut bloklara
// dokunulmaz, ama diğer alanlar öyle değil") — bu yüzden mevcut sezon
// GET'iyle scalar alanlar (posterUrl vb.) korunarak gönderilir, asla
// icat edilmez.
//
// Kullanım (PowerShell):
//   node scripts/seed-season-story.mjs --season 1                (dry-run, sadece gövdeyi yazdırır)
//   $env:AUTH_TOKEN="..."; node scripts/seed-season-story.mjs --season 1 --write
...
// paragraphs[] -> tek blok content'i (episodeBlocks'taki '\n\n' join
// konvansiyonunun aynısı, groupBlocksByScene/buildScene ile simetrik).
...
async function main() {
  const { getSeasonStory } = await import(
    pathToFileURL(resolve('src/pages/series/BreakingBad/SeasonDetail/SeasonStory/SeasonStory.data.js')).href
  );
  const en = getSeasonStory(seasonFilter);
  if (!en) throw new Error(`SeasonStory.data.js'te Season ${seasonFilter} içeriği yok.`);

  const trAll = JSON.parse(readFileSync(resolve('scripts/data/breaking-bad-season-story-tr.json'), 'utf8'));
  const tr = trAll[String(seasonFilter)];
  if (!tr) throw new Error(`breaking-bad-season-story-tr.json'da Season ${seasonFilter} çevirisi yok.`);
  ...
  writeFileSync(resolve(`scripts/output/season-${seasonFilter}-story-body.json`), JSON.stringify(body, null, 2));
  console.log(`\nGövde yazıldı: scripts/output/season-${seasonFilter}-story-body.json (${body.seasonBlocks.length} blok)`);
