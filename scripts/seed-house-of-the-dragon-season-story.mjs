#!/usr/bin/env node
// seed-season-story.mjs'in House of the Dragon varyantı — aynı desen,
// sadece SERIES_SLUG, EN kaynak (HouseOfTheDragon/SeasonDetail/SeasonStory)
// ve TR dosyası (house-of-the-dragon-season-story-tr.json) değişti.
//
// Kullanım (PowerShell):
//   node scripts/seed-house-of-the-dragon-season-story.mjs --season 1                (dry-run)
//   $env:AUTH_TOKEN="..."; node scripts/seed-house-of-the-dragon-season-story.mjs --season 1 --write
...
const SERIES_SLUG = process.env.SERIES_SLUG || 'house-of-the-dragon';
...
async function main() {
  const { getSeasonStory } = await import(
    pathToFileURL(resolve('src/pages/series/HouseOfTheDragon/SeasonDetail/SeasonStory/SeasonStory.data.js')).href
  );
  const en = getSeasonStory(seasonFilter);
  if (!en) throw new Error(`SeasonStory.data.js'te Season ${seasonFilter} içeriği yok.`);

  const trAll = JSON.parse(readFileSync(resolve('scripts/data/house-of-the-dragon-season-story-tr.json'), 'utf8'));
  const tr = trAll[String(seasonFilter)];
  if (!tr) throw new Error(`house-of-the-dragon-season-story-tr.json'da Season ${seasonFilter} çevirisi yok.`);
  ...
  writeFileSync(resolve(`scripts/output/hotd-season-${seasonFilter}-story-body.json`), JSON.stringify(body, null, 2));
  console.log(`\nGövde yazıldı: scripts/output/hotd-season-${seasonFilter}-story-body.json (${body.seasonBlocks.length} blok)`);
