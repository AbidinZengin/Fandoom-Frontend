...
Her block bir CSS class alır — **kullanıcı düzeltmesi:** `.block1`/`.block2`
gibi anlamsız sıra numarası DEĞİL, tipe göre kendi sayacıyla okunabilir isim
(`.textBlock1`, `.imageBlock2`, `.rectangleBlock1`, ...) — sonradan elle
müdahaleyi hızlandırır. Layout artık breakpoint-bazlı olduğu için
pozisyon/boyut gerçek `@media` kurallarına çevrilir (`base` → düz kural,
`md`/`lg` → `@media`). Stil matrisi (renk/efekt) + `customCss` aynı
mantıkla, `:hover` dahil, eklenir. Component tipine göre JSX: TEXT →
metin/binding taşıyan `<p>`, IMAGE → `<img>`, RECTANGLE/DIAMOND/CIRCLE →
saf `<div>` (clip-path/border-radius CSS'te). Metin/URL değerleri JSX
ifadesi (`{...}`) olarak güvenli kaçışlanarak gömülür.

**Katman sırası (z-index/DOM order) — kullanıcı uyarısı:** Tuvalde üst üste
binen block'lar olabiliyor (`bringToFront`/`blockOrder` ile kontrol
ediliyor). Üretilen JSX'te block'lar **`blockOrder` dizisiyle AYNI sırada**
emit edilir (DOM'da sonra gelen üstte durur — tarayıcının doğal stacking
kuralı) VE block'un `styles`'ında açıkça bir `zIndex` set edilmişse bu da
CSS'e aynen yazılır. İkisi birlikte tuvaldeki görsel katman sırasını birebir
korur — aksi hâlde altta duran bir görsel üretilen sayfada metnin üstüne
çıkabilirdi.

**Dinamik import yolları — kullanıcı uyarısı:** Üretilen dosya
`src/pages/series/GameOfThrones/Intro/IntroBanner/IntroBanner.jsx` gibi
derin bir klasörde olabilir. `src/shared/api/*` gibi ortak modüllere giden
`import` yollarındaki `../` sayısı SABİT değil — hedef klasörün
`src/pages/`'e göre derinliğinden (+ component'in kendi klasörü) anlık
hesaplanır (basit bir `path.relative` hesabı, elle güncellenmez).
...
kavramı icat edilmez) — **kullanıcı isteğiyle** hemen üstüne bir
`// TODO: Replace null with a Skeleton UI if needed.` yorumu eklenir, teslim
alan geliştirici ne yapması gerektiğini anında görür.
