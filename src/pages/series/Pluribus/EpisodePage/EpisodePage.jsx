...
import { theme, resolveGenreNames } from '../Pluribus.data';
...
// korundu. Platform-marka rozeti (HotD'deki HBO Max) YOK — Pluribus (Apple
...
export default function EpisodePage() {
  ...
  // Pluribus tema rengini basar (learned-rules: "Yapım sayfaları TAM TEMA kurar").
  useEffect(() => {
    if (!series) return undefined;
  ...
  const goToSeason = (delta, landOnLast = false) => {
    ...
    fetchSeasonDetail(target.id).then((detail) => {
      ...
      navigate(`/series/pluribus/seasons/${target.seasonNumber}/episodes/${num}`);
    });
  };
  ...
  const goToEpisode = (num) => {
    ...
    armInPageNav();
    navigate(`/series/pluribus/seasons/${seasonNumber}/episodes/${num}`);
  };
