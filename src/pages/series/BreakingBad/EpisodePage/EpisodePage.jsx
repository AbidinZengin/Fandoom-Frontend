...
import { theme, resolveGenreNames } from '../BreakingBad.data';
...
// GoT'un EpisodePage'inin BİREBİR aynısı (kullanıcı kararı, 2026-08) — cover
// card → fixed header scroll dönüşümü, cinematic merkez-reveal girişi,
// bölüm/sezon crossfade navigasyonu dahil TÜM mekanik korundu. Tek kasıtlı
...
export default function EpisodePage() {
  ...
  // BB tema rengini basar (learned-rules: "Yapım sayfaları TAM TEMA kurar").
  useEffect(() => {
    if (!series) return undefined;
  ...
  const goToSeason = (delta, landOnLast = false) => {
    ...
    fetchSeasonDetail(target.id).then((detail) => {
      ...
      navigate(`/series/breaking-bad/seasons/${target.seasonNumber}/episodes/${num}`);
    });
  };
  ...
  const goToEpisode = (num) => {
    ...
    armInPageNav();
    navigate(`/series/breaking-bad/seasons/${seasonNumber}/episodes/${num}`);
  };
