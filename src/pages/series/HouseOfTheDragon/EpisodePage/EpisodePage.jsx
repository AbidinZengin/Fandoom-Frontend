...
import { theme, resolveGenreNames } from '../HouseOfTheDragon.data';
...
// BreakingBad/EpisodePage'in BİREBİR aynısı (kullanıcı kararı) — cover card →
// fixed header scroll dönüşümü, cinematic merkez-reveal girişi, bölüm/sezon
// crossfade navigasyonu dahil TÜM mekanik korundu. Tek kasıtlı fark: HotD
// HBO yapımı olduğu için EpisodeBrief'te GoT'taki gibi HBO Max rozeti VAR
// (bkz. EpisodeBrief) — BB'de bu rozet AMC asset'i olmadığı için yoktu.
export default function EpisodePage() {
  ...
  // HotD tema rengini basar (learned-rules: "Yapım sayfaları TAM TEMA kurar").
  useEffect(() => {
    if (!series) return undefined;
  ...
  const goToSeason = (delta, landOnLast = false) => {
    ...
    fetchSeasonDetail(target.id).then((detail) => {
      ...
      navigate(`/series/house-of-the-dragon/seasons/${target.seasonNumber}/episodes/${num}`);
    });
  };
  ...
  const goToEpisode = (num) => {
    ...
    armInPageNav();
    navigate(`/series/house-of-the-dragon/seasons/${seasonNumber}/episodes/${num}`);
  };
