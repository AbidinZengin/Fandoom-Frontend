...
import { theme, resolveGenreNames } from '../Severance.data';
...
// korundu. Platform-marka rozeti (HotD'deki HBO Max) YOK — Severance (Apple
...
export default function EpisodePage() {
  ...
  // Severance tema rengini basar (learned-rules: "Yapım sayfaları TAM TEMA kurar").
  useEffect(() => {
    if (!series) return undefined;
  ...
  const goToSeason = (delta, landOnLast = false) => {
    ...
    fetchSeasonDetail(target.id).then((detail) => {
      ...
      navigate(`/series/severance/seasons/${target.seasonNumber}/episodes/${num}`);
    });
  };
  ...
  const goToEpisode = (num) => {
    ...
    armInPageNav();
    navigate(`/series/severance/seasons/${seasonNumber}/episodes/${num}`);
  };
