...
import { theme, resolveGenreNames } from '../ItWelcomeToDerry.data';
...
// korundu. Platform-marka rozeti (HotD'deki HBO Max) YOK — IT: Welcome to
// Derry (HBO) için eşdeğer bir logo asset'i verilmedi, bkz. EpisodeBrief.
export default function EpisodePage() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetchProductionDetail('series', 'it-welcome-to-derry')
      .then((data) => {
  ...
  // IT: Welcome to Derry tema rengini basar (learned-rules: "Yapım sayfaları TAM TEMA kurar").
  useEffect(() => {
    if (!series) return undefined;
  ...
  const goToSeason = (delta, landOnLast = false) => {
    ...
    fetchSeasonDetail(target.id).then((detail) => {
      ...
      navigate(`/series/it-welcome-to-derry/seasons/${target.seasonNumber}/episodes/${num}`);
    });
  };
  ...
  const goToEpisode = (num) => {
    ...
    armInPageNav();
    navigate(`/series/it-welcome-to-derry/seasons/${seasonNumber}/episodes/${num}`);
  };
