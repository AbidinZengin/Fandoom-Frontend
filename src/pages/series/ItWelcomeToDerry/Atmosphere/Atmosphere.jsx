...
// 2026-09-12) — kendi arka planı YOK, ItWelcomeToDerry.jsx'teki .intro
// (Hero ile PAYLAŞILAN tek görsel) bu section'ın arkasından geçiyor;
// overlay (opacity 0→1) scroll'la siyaha kararır; kararma bitmeden hemen
// önce bu bölümün kendi içeriği (eyebrow/başlık/istatistikler) belirir.
// Accent renk YOK — vurgulu metinler arkadaki görselin kendisiyle
// dolduruluyor (background-clip:text). Yazı grupları soldan açığa çıkar,
// background-position onUpdate'te HER karede yeniden hesaplanır.
export default function Atmosphere() {
  ...
  useEffect(() => {
    fetchProductionById('series', 10).then(setSeries);
  }, []);
  ...
  return (
    <section className={styles.atmosphere} ref={sectionRef}>
      ...
      >
        <p ref={eyebrowRef} className={styles.atmosphere__eyebrow}>A Town That Remembers</p>
        <h2 ref={headlineRef} className={styles.atmosphere__headline}>
          Long before the clown came back, Derry was already keeping its secrets — and its dead.
        </h2>
