...
import { theme, resolveGenreNames } from '../StrangerThings.data';
...
// korundu. Platform-marka rozeti (HotD'deki HBO Max) YOK — Stranger Things
// (Netflix) için eşdeğer bir logo asset'i verilmedi, bkz. EpisodeBrief.
export default function EpisodePage() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetchProductionDetail('series', 'stranger-things')
      .then((data) => {
  ...
  // Stranger Things tema rengini basar (learned-rules: "Yapım sayfaları TAM TEMA kurar").
  useEffect(() => {
    if (!series) return undefined;
  ...
  const goToSeason = (delta, landOnLast = false) => {
    ...
    fetchSeasonDetail(target.id).then((detail) => {
      ...
      navigate(`/series/stranger-things/seasons/${target.seasonNumber}/episodes/${num}`);
    });
  };
  ...
  const goToEpisode = (num) => {
    ...
    armInPageNav();
    navigate(`/series/stranger-things/seasons/${seasonNumber}/episodes/${num}`);
  };
