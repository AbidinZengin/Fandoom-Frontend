...
export default function Hero() {
  const [series11, setSeries11] = useState(null);
  const [genreNames, setGenreNames] = useState([]);

  useEffect(() => {
    fetchProductionById('series', 11).then(setSeries11);
  }, []);
  ...
  useEffect(() => {
    if (!series11?.genreIds?.length) {
      setGenreNames([]);
      return undefined;
    }
    let cancelled = false;
    resolveGenreNames(series11.genreIds).then((names) => {
  ...
  if (series11 == null) return null;

  const year = series11?.firstAirDate ? new Date(series11.firstAirDate).getFullYear() : '';

  return (
    <div className={styles.page}>
      <img className={styles.imageBlock1} src={series11?.posterUrl} alt="" />
      <div className={styles.cardWrap}>
        <img className={styles.imageBlock2} src={series11?.coverImageUrl} alt="" />
        <Link to="/series/stranger-things" className={styles.brandLogo}>
          <img src="/src/assets/logos/stranger-things.webp" alt="Stranger Things" />
        </Link>
        <p className={styles.textBlock1}>{series11?.title}</p>
        <div className={styles.metaRow}>
          <p className={styles.textBlock2}>{year}</p>
          <p className={styles.textBlock3}>{genreNames.join(' · ')}</p>
        </div>
        <p className={styles.textBlock4}>{series11?.synopsis}</p>
        <div className={styles.logoBlock1}><img src="/src/assets/logos/IMDB_Logo_2016.svg.webp" alt="IMDB Logo 2016.Svg" /></div>
        <p className={styles.textBlock5}>{series11?.externalRating}</p>
