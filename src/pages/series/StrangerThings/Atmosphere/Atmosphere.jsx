...
// Severance Atmosphere'inin birebir portu (kullanıcı isteği, 2026-09-12).
// Kendi arka planı YOK — StrangerThings.jsx'teki .intro__backdrop (Hero ile
// PAYLAŞILAN tek görsel) bu section'ın arkasından geçiyor; overlay (opacity
// 0→1) scroll'la siyaha kararır; kararma bitmeden hemen önce eyebrow/başlık/
// istatistikler soldan açığa çıkar. Accent renk YOK — vurgulu metinler
// arkadaki görselin kendisiyle dolduruluyor (background-clip:text).
//
// DENEYSEL VERİ NOTU: Network alanı backend şemasında yok — sabit/doğru
// bilgi olarak elle girildi (Netflix). Episode sayısı backend'de series
// detail'da toplu gelmiyor — her sezonu ayrı ayrı çekip episodes.length
// toplanıyor.
export default function Atmosphere() {
  ...
  useEffect(() => {
    fetchProductionById('series', 11).then(setSeries);
  }, []);
  ...
  // applySharedBgRef'e de atanıyor ki GSAP timeline'ı eyebrow/stats soldan
  ...
  return (
    <section className={styles.atmosphere} ref={sectionRef}>
      ...
      >
        <p ref={eyebrowRef} className={styles.atmosphere__eyebrow}>Something Is Wrong in Hawkins</p>
        <h2 ref={headlineRef} className={styles.atmosphere__headline}>
          A quiet town, a door to somewhere else, and the kids who won't stop looking for the truth.
        </h2>
